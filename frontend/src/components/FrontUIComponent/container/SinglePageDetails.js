...
function SinglePageDetails() {
    ...
    useEffect(() => {
        ...
        if (data && !loading) {
            const foundAnnouncement = data.find((ann) => ann.id === parseInt(id));
            setAnnouncement(id);
        }
        // console.log(id)
    }, []);
        ...
        <div className="container single-page-details">
            {loading ? (
                ...
                <>
                    ...
                    <div className="card">
                        <img src={announcement.image?.url} className="card-img-top" alt="..." />
                        <div className="card-body">
                            <h5 className="card-title">{announcement.name}</h5>
                            <p className="card-text">{announcement.desc}</p>
                            <p className="card-text">
                                <small className="text-muted">Date: {moment(announcement.date).format("MM/DD/YYYY")}</small>
                            </p>
                        </div>
                    </div>
                </>
